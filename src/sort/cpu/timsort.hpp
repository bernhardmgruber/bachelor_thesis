...
#include <iostream>
#define LOG(expr) (clog << "# " << __func__ << ": " << expr << endl)
#else
...
class TimSortImpl {
    ...
    typedef typename iterator_traits<iter_t>::reference ref_t;
    typedef typename iterator_traits<iter_t>::difference_type diff_t;
    ...
    vector<value_t> tmp_; // temp storage for merges
    typedef typename vector<value_t>::iterator tmp_iter_t;

    vector<iter_t> runBase_;
    vector<diff_t> runLen_;

    TimSortImpl(iter_t const lo, iter_t const hi, compare_t c)
            :
    ...
    void mergeLo(iter_t base1, diff_t len1, iter_t base2, diff_t len2) {
        ...
        copy(base1, base1 + len1, back_inserter(tmp_));
        ...
        *(dest++) = *(cursor2++);
        if(--len2 == 0) {
            copy(cursor1, cursor1 + len1, dest);
            return;
        }
        if(len1 == 1) {
            copy(cursor2, cursor2 + len2, dest);
            *(dest + len2) = *cursor1;
            return;
        }
        ...
        while(true) {
            ...
            do {
                ...
                if(count1 != 0) {
                    copy(cursor1, cursor1 + count1, dest);
                    dest    += count1;
                    cursor1 += count1;
                    len1    -= count1;
                ...
                *(dest++) = *(cursor2++);
                ...
                if(count2 != 0) {
                    copy(cursor2, cursor2 + count2, dest);
                    dest    += count2;
                    cursor2 += count2;
                    len2    -= count2;
                    if(len2 == 0) {
    ...
    void mergeHi(iter_t base1, diff_t len1, iter_t base2, diff_t len2) {
        ...
        copy(base2, base2 + len2, back_inserter(tmp_));
        ...
        *(dest--) = *(cursor1--);
        if(--len1 == 0) {
            copy(tmp_.begin(), tmp_.begin() + len2, dest - (len2 - 1));
            return;
        }
        if(len2 == 1) {
            dest    -= len1;
            cursor1 -= len1;
            copy(cursor1 + 1, cursor1 + (1 + len1), dest + 1);
            *dest = *cursor2;
            return;
        }
        ...
        while(true) {
            ...
            do {
                ...
                if(count1 != 0) {
                    ...
                    cursor1 -= count1;
                    len1    -= count1;
                    copy(cursor1 + 1, cursor1 + (1 + count1), dest + 1);
                ...
                *(dest--) = *(cursor2--);
                ...
                if(count2 != 0) {
                    ...
                    cursor2 -= count2;
                    len2    -= count2;
                    copy(cursor2 + 1, cursor2 + (1 + count2), dest + 1);
                    if(len2 <= 1) {
                        break_outer = true;
                        break;
                    }
...
static inline void timsort(Iter first, Iter last, Less c) {
    TimSortImpl<Iter, Less>::sort(first, last, c);
}
