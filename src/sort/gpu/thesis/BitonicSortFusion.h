    ...
    namespace thesis
    {
        ...
        class BitonicSortFusion : public CLAlgorithm<T>, public SortAlgorithm
        {
            ...
            public:
                ...
                {
                    return "Bitonic sort fusion (THESIS dixxi Bealto)";
                }
                ...
                void init() override
                {
                    Program* program = context->createProgram("gpu/thesis/BitonicSortFusion.cl", "-D T=" + getTypeName<T>());
                ...
                void run(size_t workGroupSize, size_t size) override
                {
                    for (cl_uint startInc = 1; startInc < bufferSize; startInc <<= 1)
                    {
                        for (cl_uint inc = startInc; inc > 0; )
                        {
                            ...
                            // kernel32 is highly inefficient as it needes scratch registers
                            //if (inc >= 16 && ninc == 0)
                            //{
                            //    kernel = kernel32;
                            //    ninc = 5;
