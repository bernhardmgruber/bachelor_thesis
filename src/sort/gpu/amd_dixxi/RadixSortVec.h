...
namespace gpu
{
    namespace amd_dixxi
    {
        ...
        */
        template<typename T>
        class RadixSortVec : public CLAlgorithm<T>, public SortAlgorithm
        {
            ...
            static const unsigned int FETCH_VECTOR_WIDTH = 16; // for radix kernel
        ...
        public:
            ...
            {
                return "Radix sort vec (AMD dixxi)";
            }
            ...
            {
                stringstream ss;
                ss << "-D T=" << getTypeName<T>() << " -D RADIX=" << RADIX << " -D BLOCK_SIZE=" << BLOCK_SIZE << " -D FETCH_VECTOR_WIDTH=" << FETCH_VECTOR_WIDTH;

#ifdef THREAD_HIST_IN_REGISTERS
                ss << " -D THREAD_HIST_IN_REGISTERS";
#endif

                Program* program = context->createProgram("gpu/amd_dixxi/RadixSortVec.cl", ss.str());
