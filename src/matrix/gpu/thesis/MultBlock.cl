...
__kernel void MultBlock(__global float4* a, __global float4* b, __global float4* c, uint size)
{
    ...
    {
        float4 blA0 = a[(row * BLOCK_SIZE + 0) * size4 + i];
        float4 blA1 = a[(row * BLOCK_SIZE + 1) * size4 + i];
        float4 blA2 = a[(row * BLOCK_SIZE + 2) * size4 + i];
        float4 blA3 = a[(row * BLOCK_SIZE + 3) * size4 + i];

        float4 blB0 = b[(i * BLOCK_SIZE + 0) * size4 + col];
        float4 blB1 = b[(i * BLOCK_SIZE + 1) * size4 + col];
        float4 blB2 = b[(i * BLOCK_SIZE + 2) * size4 + col];
        float4 blB3 = b[(i * BLOCK_SIZE + 3) * size4 + col];
