...
__kernel void MultBlockLocal(__global float4* a, __global float4* b, __global float4* c, uint size)
{
    ...
    uint posA = (row * BLOCK_SIZE) * size4 + localX;
    uint posB = (localY * BLOCK_SIZE) * size4 + col;
    uint stepA = TILE_SIZE;
    uint stepB = TILE_SIZE * BLOCK_SIZE * size4;
    uint endA = posA + size4;
    ...
    while(posA < endA)
    {
        ...
        aTile[tilePos + 0 * TILE_SIZE] = a[posA + 0 * size4];
        aTile[tilePos + 1 * TILE_SIZE] = a[posA + 1 * size4];
        aTile[tilePos + 2 * TILE_SIZE] = a[posA + 2 * size4];
        aTile[tilePos + 3 * TILE_SIZE] = a[posA + 3 * size4];

        bTile[tilePos + 0 * TILE_SIZE] = b[posB + 0 * size4]; 
        bTile[tilePos + 1 * TILE_SIZE] = b[posB + 1 * size4];
        bTile[tilePos + 2 * TILE_SIZE] = b[posB + 2 * size4];
        bTile[tilePos + 3 * TILE_SIZE] = b[posB + 3 * size4];
    ...
    uint posC = (row * BLOCK_SIZE) * size4 + col;
